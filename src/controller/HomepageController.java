...
public class HomePageController extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
//   slide next auto
			String sql1= "SELECT id, `name`, img, des FROM slide WHERE active = 1";
			PreparedStatement a = ConnectionDB.getPreparedStatement(sql1);
			ResultSet b= a.executeQuery(sql1);
			request.setAttribute("b",b);
///  tin cong nghe

			String sql2= "SELECT id, `name` FROM slide WHERE active = 1";
			PreparedStatement a1 = ConnectionDB.getPreparedStatement(sql2);
			ResultSet b1= a1.executeQuery(sql2);
